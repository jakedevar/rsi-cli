...
use crate::harness::api_key::{ANTHROPIC_ENV_VARS, resolve_api_key};
use crate::harness::provider::{ApiProvider, Result};
use crate::harness::sse::{AccumulatedToolCall, read_anthropic_sse_stream};
...
impl ApiProvider for AnthropicProvider {
    async fn chat(
        ...
        let resp = execution
            .bind_http(
                crate::model_control::registry::RuntimeExecutionRoute::HarnessAnthropicHttp,
                request,
            )
            .send("Anthropic")
            .await?;
    ...
    async fn stream_chat(
        ...
        let (content, tool_calls, usage, stop_reason) =
            read_anthropic_sse_stream(resp, &chunk_tx, &self.cancel).await?;
