...
//!
//! Splits long messages into chunks that fit within a configurable max length
//! (default 4000). Splits at paragraph boundaries first, then sentence
//! boundaries, then word boundaries, then hard-splits at UTF-8 char boundaries.
//! Each chunk is prefixed with `[N/M] ` when there is more than one.
...
fn split_at_boundaries(text: &str, max_len: usize) -> Vec<String> {
    ...
    while !remaining.is_empty() {
        ...
        // Hard split at max_len, respecting UTF-8 boundaries
...
mod tests {
    ...
    fn test_unicode_safe_split() {
        // Emoji at the end — should not split through a multi-byte character
        let text = "a".repeat(99) + "\u{1F600}";
        let chunks = chunk_message(&text, 100);
