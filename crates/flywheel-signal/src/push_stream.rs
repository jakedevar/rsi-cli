...
//!
//! Opens a second Unix socket connection to the daemon, sends a `Subscribe`
//! RPC, then forwards `BusEvent` JSON lines into an mpsc channel. Reconnects
//! with exponential backoff on connection loss.
//!
//! Carbon-copy of `flywheel-imessage::push_stream`; the daemon's push surface
