...
//!
//! Follows the same wire protocol as the TUI's `DaemonClient` but exposes
//! only the subset of RPC methods needed by the iMessage bridge.
...
impl DaemonClient {
    ...
    /// Get the socket path (for spawning independent connections like push stream).
    #[allow(dead_code)]
